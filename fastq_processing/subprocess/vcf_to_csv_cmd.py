...
def vcf_to_csv(path_to_vcf,output_path=None):
    ...
    keep_list = ' Gene_Name | HGVS.p | ERRORS / WARNINGS / INFO'.split("|") # keep data from provided fields
    old_list = 'MUTATION| Gene_Name | HGVS.p |P_ERR_MUT_CALL| ERRORS / WARNINGS / INFO'.split("|") #renaming vcf fields for improved readability of mutation report
    new_list = 'MUTATION|GENE|AMINO_ACID_CHANGE|P_ERR_MUT_CALL|ERRORS/WARNINGS/INFO'.split("|")
    ...
    output_data = output_data['MUTATION| Gene_Name | HGVS.p |P_ERR_MUT_CALL| ERRORS / WARNINGS / INFO'.split("|")] #drop columns used only in calculation
    ...
    output_data = output_data['MUTATION|GENE|AMINO_ACID_CHANGE|COVERAGE|FREQUENCY|P_ERR_MUT_CALL|ERRORS/WARNINGS/INFO'.split("|")] #reorder dataframe columns
